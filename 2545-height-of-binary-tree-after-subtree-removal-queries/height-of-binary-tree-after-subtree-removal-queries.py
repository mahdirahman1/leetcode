...
class Solution:
    def treeQueries(self, root: Optional[TreeNode], queries: List[int]) -> List[int]:
        # do a dfs and store the height of each subtree in a dictionary
        # also in another dictionary, store the parent of each node, 
        # this will help to easily delete the subtree from that node using the parent, can also store left or right child
